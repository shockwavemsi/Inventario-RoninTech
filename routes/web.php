...
Route::middleware(['auth'])->group(function () {
    ...
    // ✅ ENDPOINT INTELIGENTE: Decide qué mostrar basado en si hay albaranes previos
    Route::get('/api/pedidos/{id}/productos', function($id) {
        ...
        // Sumar TODOS los albaranes previos
        ...
        $lineasMap = $lineas->map(function($linea) use ($recibidoPorProducto) {
        ...
        // ✅ LÓGICA: Si ya tiene albaranes → SOLO los con faltante > 0

        if ($pedido->albaranes->count() > 0) {

            $lineasMap = $lineasMap->filter(fn($l) => $l['cantidad_faltante'] > 0);

        }

        return response()->json($lineasMap->values());

    });

    // ✅ ENDPOINT ALTERNATIVO: Retorna SOLO faltantes (sin filtro de albaranes)
