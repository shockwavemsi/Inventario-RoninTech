...

{
        ...
        try {
            ...
            return response()->json([
                ...
                'pagos' => $facturaCompra->pagos ? $facturaCompra->pagos->map(fn($p) => [
                    ...
                    'metodo_pago' => $p->forma_pago_nombre ?? '—',
...

    {
        ...
        try {
            ...
            // ✅ GUARDAR PAGOS CON FORMA Y BANCO

            if ($request->has('forma_pago_id') && is_array($request->forma_pago_id)) {

                $formasPagoNombres = $request->input('forma_pago_nombre', []);

                $bancoNombres = $request->input('banco_nombre', []);
                ...
                foreach ($request->forma_pago_id as $idx => $formaId) {
                    ...
                    if ($formaId > 0 && $monto > 0) {
                        ...
                        $nombreFormaPago = $formaPago?->formaPago?->nombre ?? $formasPagoNombres[$idx] ?? '';

                        $nombreBanco = $formaPago?->banco?->nombre ?? $bancoNombres[$idx] ?? '—';
