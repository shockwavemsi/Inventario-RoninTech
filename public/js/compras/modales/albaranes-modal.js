/**
 * Albaranes Modal - Todos los SVG Rojo #e63946
 */

import { BADGE_COLORS } from '../config.js';

class AlbaranesModal {
    ...
    static getSVG(type) {
        const svgs = {
            ...
            arrow: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#e63946" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="12" y1="5" x2="12" y2="19"></line><polyline points="19 12 12 19 5 12"></polyline></svg>`,
            ...
            check: `<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#90ee90" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="20 6 9 17 4 12"></polyline></svg>`,
    ...
    static mostrar(item) {
        const titulo = `Albarán: ${item.numero_albaran}`;

        let filas_tabla = '';
        if (item.detalles && item.detalles.length > 0) {
            filas_tabla = item.detalles.map((d, idx) => {
                const statusColor = d.estado === 'recibido' ? '#90ee90' : '#fed7aa';
                const statusBg = d.estado === 'recibido' ? 'rgba(34, 197, 94, 0.1)' : 'rgba(249, 115, 22, 0.1)';
                return `
                    <tr style="border-bottom: 1px solid rgba(230, 57, 70, 0.1);">
                        ...
                        <td style="padding: 0.75rem; color: #f0f0f0; font-weight: 500;">${d.producto_nombre || 'Producto #' + d.producto_id}</td>
                        <td style="padding: 0.75rem; color: #f0f0f0; text-align: center; font-weight: 500;">${d.cantidad_pedida}</td>
                        <td style="padding: 0.75rem; color: #90ee90; text-align: center; font-weight: 500;">${d.cantidad_recibida}</td>
                        <td style="padding: 0.75rem; color: #fed7aa; text-align: center; font-weight: 500;">${d.cantidad_faltante}</td>
                        <td style="padding: 0.75rem; text-align: center;">
                            <span style="color: ${statusColor}; background: ${statusBg}; padding: 0.25rem 0.75rem; border-radius: 4px; font-size: 0.75rem; font-weight: 600; text-transform: uppercase;">
                                ${d.estado}
                            </span>
                        </td>
                    </tr>
        ...
        const contenido = `
            ...
            <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 2rem; margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 1px solid rgba(230, 57, 70, 0.2);">
                ...
                <div>
                    ...
                    <div style="display: grid; gap: 1rem;">
                        <div>
                            <div style="font-size: 0.7rem; color: #a0a0a0; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.4rem; font-weight: 600;">Pedido Origen</div>
                            <div style="color: #f0f0f0; font-weight: 500; font-size: 1rem;">${item.numero_pedido || '—'}</div>
                        </div>
                        <div>
                            <div style="font-size: 0.7rem; color: #a0a0a0; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.4rem; font-weight: 600;">Fecha Albarán</div>
                            <div style="color: #f0f0f0; font-weight: 500;">${item.fecha_albaran || '—'}</div>
                        </div>
                    </div>
                </div>
                ...
                <div>
                    <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1.5rem;">
                        ${this.getSVG('chart')}
                        <h6 style="margin: 0; color: #e63946; font-weight: 700; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.5px;">Estado y Recepción</h6>
                    </div>
                    ...
                    <div style="margin-bottom: 1rem;">
                        <div style="font-size: 0.7rem; color: #a0a0a0; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 0.4rem; font-weight: 600;">Fecha Recepción</div>
                        <div style="color: #90ee90; font-weight: 500;">${item.fecha_recepcion || '—'}</div>
                    </div>
                </div>
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 1rem;">
                    ${this.getSVG('arrow')}
                    <h6 style="margin: 0; color: #e63946; font-weight: 700; font-size: 0.9rem; text-transform: uppercase; letter-spacing: 0.5px;">Detalle de Recepción</h6>
                </div>
                    ...
                    <table style="width: 100%; border-collapse: collapse;">
                        <thead>
                            <tr style="background: rgba(230, 57, 70, 0.15);">
                                ...
                                <th style="padding: 0.75rem; color: #e63946; text-align: center; font-weight: 700; font-size: 0.875rem;">PEDIDO</th>
                                <th style="padding: 0.75rem; color: #e63946; text-align: center; font-weight: 700; font-size: 0.875rem;">RECIBIDO</th>
                                <th style="padding: 0.75rem; color: #e63946; text-align: center; font-weight: 700; font-size: 0.875rem;">FALTANTE</th>
                                <th style="padding: 0.75rem; color: #e63946; text-align: center; font-weight: 700; font-size: 0.875rem;">ESTADO</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${filas_tabla || '<tr><td colspan="6" style="text-align: center; padding: 1rem; color: #a0a0a0;">Sin detalles</td></tr>'}
                        </tbody>
                    </table>
                </div>
            </div>
                ...
                <div id="contenedorRelaciones" style="display: grid; gap: 0.5rem;">
                    ${item.numero_pedido && item.numero_pedido !== '—' ? `
                        <div style="display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; background: rgba(230, 57, 70, 0.05); border-radius: 6px;">
                            ...
                            <span style="color: #f0f0f0;">${item.numero_pedido}</span>
                        </div>
                    ` : ''}

                    ${item.facturas && item.facturas.length ? item.facturas.map(f => `
                        <div style="display: flex; align-items: center; gap: 0.75rem; padding: 0.75rem; background: rgba(230, 57, 70, 0.05); border-radius: 6px;">
                            ${this.getSVG('document')}
        ...
        this.crearYMostrarModal(titulo, contenido);
    }

    static crearYMostrarModal(titulo, contenido) {
        const modalHTML = `
            <div class="modal fade" id="detalleAlbaranModal" tabindex="-1">
        ...
        const modalAnterior = document.getElementById('detalleAlbaranModal');
        if (modalAnterior) modalAnterior.remove();

        document.body.insertAdjacentHTML('beforeend', modalHTML);
        const modal = new bootstrap.Modal(document.getElementById('detalleAlbaranModal'));
        modal.show();

        document.getElementById('detalleAlbaranModal').addEventListener('hidden.bs.modal', function() {
            this.remove();
        });
    }
